  ...
  return (
    <Box p={4} maxW="1200px" mx="auto">
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={6}>
        テンプレートマッチング（Template
        Matching）は、入力画像全体の中から、あらかじめ用意した「テンプレート画像」（探したいパターン）と最も似ている場所を探し出す、最も基本的な物体検出手法の一つです。「ウォーリーを探せ」のように、大きな画像の中から小さな画像を探す処理をイメージすると分かりやすいでしょう。
      </Text>
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={6}>
        「似ている」かどうかを数値化するために、いくつかの計算式が使われます。以下の{" "}
        <InlineMath math="T" /> はテンプレート画像、
        <InlineMath math="I" /> は入力画像、
        <InlineMath math="(x,y)" /> は現在の走査位置、
        <InlineMath math="(x',y')" /> はテンプレート内の座標を表します。
      </Text>
      ...
      <Box mb={8}>
        ...
        <Text fontSize="md" lineHeight="1.8" mb={4}>
          画素値の差の二乗和です。値が<strong>小さいほど似ている</strong>
          ことになります。計算が単純で高速ですが、照明の変化（全体の明るさの違いなど）に弱いです。
        </Text>
        <Box mb={4}>
      ...
      <Box mb={8}>
        ...
        <Text fontSize="md" lineHeight="1.8" mb={4}>
          正規化相互相関です。ベクトルの内積のような計算を行い、正規化します。値は
          -1 から 1 の数値をとり、<strong>1に近いほど似ている</strong>
          ことになります。SSDに比べて計算量は増えますが、照明の変化（明るさの変動）に対して強くなります。
        </Text>
        <Box mb={4}>
        ...
        <Text fontSize="md" lineHeight="1.8" mb={4}>
          さらに平均値を引いてから計算する{" "}
          <strong>ZNCC (Zero-mean Normalized Cross Correlation)</strong>{" "}
          は、さらに照明変化に強固になりますが、計算コストは最も高くなります。
        </Text>
      </Box>
      ...
      <Box p={6} borderWidth="1px" borderRadius="lg" mb={8} bg="gray.50">
        <List spacing={3}>
          ...
          <ListItem mt={4}>
            ...
            <List styleType="circle" pl={6} mt={2}>
              <ListItem>
                <strong>回転、スケール変化、変形に弱い。</strong>
                （テンプレートと全く同じ向き・大きさでないと検出できない）
              </ListItem>
              <ListItem>
                遮蔽（オクルージョン）があるとうまく検出できない。
              </ListItem>
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={4}>
        OpenCVでは <code>cv2.matchTemplate</code>{" "}
        関数を使って簡単に実装できます。
      </Text>
...
cv2.destroyAllWindows()`}
      ...
      <List spacing={4} styleType="decimal" pl={4} mb={8}>
        <ListItem>
          ...
          <Text fontSize="md" lineHeight="1.8">
            単一のフレームだけで判断すると、ノイズや一時的な遮蔽によって検出位置が大きく飛んでしまうことがあります。これを防ぐために、
            <strong>「過去数フレームの結果を統合する」</strong>手法が有効です。
            <br />
        ...
        <ListItem>
          ...
          <Text fontSize="md" lineHeight="1.8">
            ...
            そこで、
            <strong>
              「マッチングに成功したら、その場所の画像を新しいテンプレートとして更新する」
            </strong>
            というアプローチがあります。ただし、少しずつズレた画像を更新し続けると、最終的に別の物体を追跡してしまう（ドリフト現象）リスクがあるため、初期テンプレートとの類似度も併用して監視するなどの対策が必要です。
          </Text>
        </ListItem>
        ...
        <ListItem>
          ...
          <Text fontSize="md" lineHeight="1.8">
            対象物が回転したり変形したりすることが予想される場合、あらかじめ
            <strong>「少しずつ角度や大きさを変えた複数のテンプレート」</strong>
            を用意しておく手法です。
            <br />
...
template = frame[y:y+h, x:x+w]`}
    ...
    if cv2.waitKey(30) & 0xFF == 27: break`}
        ...
        <ListItem>
          ...
          <List spacing={3} styleType="disc" pl={4}>
            <ListItem>
              ...
              <Text fontSize="md">
                `max_val` が一定値（例:
                0.8）以下なら「見つからなかった」と判断し、矩形を描画しないようにします。
              </Text>
            </ListItem>
            <ListItem>
              <Text fontSize="md" fontWeight="bold">
              ...
              <Text fontSize="md">
                高いスコア（例:
                0.9以上）で検出できた場合のみ、その切り抜き画像を「新しいテンプレート」として上書き保存します。これにより、徐々に変化する見た目に対応できます。
              </Text>
            </ListItem>
          </List>
        </ListItem>
      ...
      <List spacing={4} styleType="decimal" pl={4} mb={8}>
        ...
        <ListItem>
          <Text fontSize="md" lineHeight="1.8">
            <code>cv2.matchTemplate</code> の第3引数（手法）を{" "}
            <code>cv2.TM_SQDIFF</code> (SSD)
            に変更し、結果の違いを確認してください。SSDの場合は、最大値ではなく
            <strong>最小値</strong>の場所が正解になる点に注意しましょう。
          </Text>
        </ListItem>
        <ListItem>
          <Text fontSize="md" lineHeight="1.8">
