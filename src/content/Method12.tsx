  ...
  return (
    <Box p={4} maxW="1200px" mx="auto">
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={6}>
        画像処理やロボティクスにおいて、カメラやセンサから得られるデータには常に
        <strong>ノイズ</strong>が含まれています。
        また、対象物が隠れて見えなくなる（オクルージョン）こともあります。
        このような不確実な状況下で、対象物の真の位置や速度（状態）を推定するために用いられるのが
        <strong>ベイジアンフィルタ</strong>や<strong>カルマンフィルタ</strong>
        です。
      </Text>
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={6}>
        「ボールは座標(100, 200)にある」と断定するのではなく、「座標(100,
        200)にある確率が一番高いが、(101, 200)かもしれない」というように、
        状態を<strong>確率分布</strong>
        として扱います。これにより、センサの誤差や予測の曖昧さを数学的に統合することができます。
      </Text>
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={4}>
        時系列フィルタは、主に「<strong>予測（Prediction）</strong>」と「
        <strong>更新（Update）</strong>」の2つのステップを繰り返します。
      ...
      <UnorderedList spacing={2} pl={4} mb={6}>
        <ListItem>
          システムが<strong>線形</strong>である（行列の掛け算で表せる）
        </ListItem>
        <ListItem>
          ノイズが<strong>ガウス分布（正規分布）</strong>に従う
        </ListItem>
      </UnorderedList>
      ...
      <Box p={4} borderWidth="1px" borderRadius="md" mb={6}>
        ...
        <Text fontSize="md" lineHeight="1.8">
          カルマンフィルタの核心は「予測をどれくらい信じるか」と「観測をどれくらい信じるか」の重み付けを自動調整する
          <strong>カルマンゲイン</strong>にあります。
          <br />
          <UnorderedList mt={2}>
            <ListItem>
              センサのノイズが少ない → 観測を重視（
      ...
      <Text fontSize="md" fontFamily="Verdana" lineHeight="1.8" mb={6}>
        ...
        <br />
        そこで、確率分布を数式ではなく、
        <strong>大量の粒子（パーティクル）</strong>
        の集まりで近似する方法がパーティクルフィルタです。
      ...
      <List spacing={3} styleType="decimal" pl={4} mb={8}>
        ...
        <ListItem>
          OpenCVの `cv2.KalmanFilter`
          を使用して、マウスカーソルの動きを追跡（スムージング）するプログラムを作成しなさい。
        </ListItem>
        <ListItem>
          <strong>応用課題:</strong>{" "}
          動画内の物体追跡（トラッキング）において、対象が一時的に遮蔽物（オクルージョン）に隠れた際、カルマンフィルタの「予測」がどのように役立つか考察しなさい。
        </ListItem>
      </List>
    </Box>
  );
